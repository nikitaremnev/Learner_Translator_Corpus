...
class DocumentAdmin(admin.ModelAdmin):
    fieldsets = [  # в панели редактирования
        ...
        ('Date', {'fields': [('date1')]}),
        ('Text', {'fields': [('genre','time_limit', 'mode')]}),
        ('Autocompletion', {'fields': [('annotated', 'checked')], 'classes': [('collapse')]}),
    ]

    list_display = ('id', 'title', 'owner', 'author', 'gender', 'native', 'language_background', 'level', 'mode', 'created', 'annotated', 'checked', 'fullmeta')  # в таблице
    list_filter = ['fullmeta', OwnerFilter, 'gender', 'annotated', 'checked', 'level', 'native', 'language_background', 'course', 'genre', 'mode']  # фильтры справа в панели редактирования
